"""
API Test
Delete an article - Delete request(without url_params)
...
def test_api_example(test_api_obj):
    ...
    try:
        ...
        # Delete an article
        response = test_api_obj.delete_article(article_details=article_details,
                                            headers=headers)
        result_flag = True if 'Success' in response.get('response',{}).get('Message','fail') else False
        test_api_obj.log_result(result_flag,
                                positive='Successfully deleted article with details %s' % response,
                                negative='Could not delete new article with details %s' % response)
