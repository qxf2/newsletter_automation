...
class ArticleForm(FlaskForm):
    ...
    subject = StringField('Subject',render_kw={'placeholder': 'Just the date in dd-mmm-yyyy'}, validators=[DataRequired(message="Subject cannot be blank"), Regexp('^\d\d-[A-Z][a-z][a-z]-\d\d\d\d', message='Invalid Subject. Subject should be of form dd-MMM-YYY E.g.: 15-Aug-1947'), Length(min=11, max=11, message='Subject should be exactly 11 characters long. Subject should be of form dd-MMM-YYY E.g.: 15-Aug-1947')])
    added_articles = my_field = TextAreaField('Added Articles:', render_kw={'readonly': True})
    opener = TextAreaField('Opener',render_kw={'placeholder': 'Do NOT include the string "In this issue: "'}, validators=[DataRequired(message="Opener cannot be blank"), Length(min=150, max=400, message='Opener should be between 150 and 399 characters long')])
    preview_text = TextAreaField('Preview Text',render_kw={'maxlength': 150}, validators=[DataRequired(message="Preview text cannot be blank"), Length(min=70, max=150, message='Preview should be between 70 and 150 characters long')])
